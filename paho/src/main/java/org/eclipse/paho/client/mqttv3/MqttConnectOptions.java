...
public class MqttConnectOptions {
     ...
     *
     * @throws IllegalArgumentException if the user name is blank or only contains whitespace
     * characters.
     */
	 ...
	 * unexpectedly loses its connection to the server, the server will publish a message
	 ...
	 * interval between messages sent or received. It enables the client to detect if the
     * server is no longer available, without having to wait for the TCP/IP timeout. The client
     * will ensure that at least one message travels across the network within each keep
     * alive period. In the absence of a data-related message during the time period, the client
     * sends a very small "ping" message, which the server will acknowledge. A value of 0
    ...
    /**
     * Sets the connection timeout value. This value, measured in seconds, defines the maximum
     * time interval the client will wait for the network connection to the MQTT server to
     * be established. The default timeout is 30 seconds. A value of 0 disables timeout processing
     * meaning the client will wait until the network connection is made successfully or
     * fails.
     *
     ...
     *
     * @param handshakeTimeout the timeout value, measured in seconds. It must be >0;
     */
    ...
    /**
     * Sets the read timeout value. This value, measured in seconds, defines the maximum time
	 * interval a read() call on the InputStream associated with this Socket will block for.
     * The default timeout is 360 seconds. A value of 0 is interpreted as an infinite timeout.
     *
     * @param readTimeout the timeout value, measured in seconds. It must be >0;
     */
    ...
    /**
     * Returns the socket factory that will be used when connecting, or <code>null</code> if one
     * has not been set.
     */
    ...
    /**
     * Sets the <code>SocketFactory</code> to use. This allows an application to apply its own
     * policies around the creation of network sockets. If using an SSL connection, an
    ...
    /**
     * Returns the message to be sent as last will and testament (LWT). The returned object is
     * "read only". Calling any "setter" methods on the returned object will result in an
    ...
    /**
     * Sets the SSL properties for the connection. Note that these properties are only valid if
	 * an implementation of the Java Secure Socket Extensions (JSSE) is available. These
     * properties are <em>not</em> used if a SocketFactory has been set using
	 * {@link #setSocketFactory(SocketFactory)}. The following properties can be used:</p>
     * <dl>
     ...
     * <dd>The name of the file that contains the KeyStore object that you want the KeyManager to
     * use. For example /mydir/etc/key.p12</dd>
     *
     * <dt>com.ibm.ssl.keyStorePassword</dt>
     * <dd>The password for the KeyStore object that you want the KeyManager to use. The password
     * can either be in plain-text, or may be obfuscated using the static method:
     * <code>com.ibm.micro.security.Password.obfuscate(char[] password)</code>. This obfuscates
     * the password using a simple and insecure XOR and Base64 encoding mechanism. Note
     ...
     * <dd>The name of the file that contains the KeyStore object that you want the TrustManager
     * to use.</dd>
     *
     * <dt>com.ibm.ssl.trustStorePassword</dt>
     * <dd>The password for the TrustStore object that you want the TrustManager to use. The
     ...
     * <dd>The type of KeyStore object that you want the default TrustManager to use. Same
     * possible values as "keyStoreType".</dd>
     *
     ...
     * <dd>Sets the algorithm that will be used to instantiate a KeyManagerFactory object instead
     * of using the default algorithm available in the platform. Example values:
     ...
     * instead of using the default algorithm available in the platform. Example values: "PKIX"
     * or "IBMJ9X509".</dd>
     * </dl>
     */
    ...
    /**
     * Returns whether the client and server should remember state for the client across
     * reconnects.
     *
     ...
     * <ul>
     * <li>Message delivery will be reliable meeting the specified QOS even if the client, server
	 * or connection are restarted.
     * <li>The server will treat a subscription as durable.
     * </ul>
     * <lI>If set to true the client and server will not maintain state across restarts of the
     * client, the server or the connection. This means
     * <ul>
     ...
     * <p>
     * Each <code>serverURI</code> specifies the address of a server that the client may connect
     * to. Two types of connection are supported <code>tcp://</code> for a TCP connection
     * and <code>ssl://</code> for a TCP connection secured by SSL/TLS. For example:
     * <ul>
     * <li><code>tcp://localhost:1883</code></li>
     * <li><code>ssl://localhost:8883</code></li>
     * </ul>
     * If the port is not specified, it will default to 1883 for <code>tcp://</code>" URIs, and
     * 8883 for <code>ssl://</code> URIs.
     * <p>
     * If serverURIs is set then it overrides the serverURI parameter passed in on the constructor
     * of the MQTT client.
     * <p>
     * When an attempt to connect is initiated the client will start with the first serverURI in
     * the list and work through the list until a connection is established with a server.
     ...
     * <p>
     * Some MQTT servers support a high availability feature where two or more "equal" MQTT
     * servers share state. An MQTT client can connect to any of the "equal" servers and be
     * assured that messages are reliably delivered and durable subscriptions are maintained no
     * matter which server the client connects to.
     * <p>
     * The cleansession flag must be set to false if durable subscriptions and/or reliable message
     * delivery is required.
     * <li>Hunt List
     * <p>
     * A set of servers may be specified that are not "equal" (as in the high availability option)
     * . As no state is shared across the servers reliable message delivery and durable
     * subscriptions are not valid. The cleansession flag must be set to true if the hunt list
     * mode is used
     * </ol>
     * </p>
     *
