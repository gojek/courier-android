...
public class CommsCallbackNew implements Runnable, ICommsCallback
{
	...
	final static String className = CommsCallbackNew.class.getName();

	private final String TAG = "COMMSCALLBACK";

	CommsCallbackNew(ClientComms clientComms, ILogger logger)
	{
		this.clientComms = clientComms;
		this.logger = logger;
		this.queue = new Vector(INBOUND_QUEUE_SIZE);
	}
				...
				if (running)
				{
					...
					synchronized (queue)
					{
						if (!queue.isEmpty())
						{
							...
							token = queue.elementAt(0);
							queue.removeElementAt(0);
						}
					}
					if (null != token)
					{
						if(token instanceof MqttToken) {
							handleActionComplete((MqttToken) token);
						} else if (token instanceof  MqttPublish) {

							handleMessage((MqttPublish) token);
						}
					}
				}
	...
	public void messageArrived(MqttPublish sendMessage)
	{
		...
		if (mqttCallback != null)
		{
			...
			synchronized (spaceAvailable)
			{
				while (!quiescing && queue.size() >= INBOUND_QUEUE_SIZE)
				{
					try
					{
