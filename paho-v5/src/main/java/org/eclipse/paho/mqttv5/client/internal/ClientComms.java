...
import org.eclipse.paho.mqttv5.client.MqttInterceptor;
...
public class ClientComms {
    ...
    public ClientComms(
            ...
            MqttSessionState mqttSession,
            MqttConnectionState mqttConnection
    ) throws MqttException {
        this(client, persistence, pingSender, mqttSession, mqttConnection,
                10, new NoOpLogger(), null, null, new NoOpsPahoEvents());
    }

    /**
     * Creates a new ClientComms object with Courier customizations (logging, telemetry,
     * experiments and packet interception).
     */
    public ClientComms(
            MqttClientInterface client,
            MqttClientPersistence persistence,
            ...
            IPahoEvents pahoEvents) throws MqttException {
        ...
        this.callback = new CommsCallback(this, this.logger);
        this.clientState = new ClientState(
                ...
                experimentsConfig,
                this.pahoEvents);
        this.mqttInterceptorCallback = new MqttInterceptorCallback(mqttInterceptorList,
                this.logger);
    ...
    void internalSend(MqttWireMessage message, MqttToken token) throws MqttException {
        ...
        // log.fine(CLASS_NAME, methodName, "200", new Object[] { message.getKey(), message,
        // token });
        ...
        if (token.getClient() == null) {
            ...
            // log.fine(CLASS_NAME, methodName, "213", new Object[] { message.getKey(), message,
            // token });
    ...
    public void sendNoWait(MqttWireMessage message, MqttToken token) throws MqttException {
        ...
        if (isConnected() ||
            ...
            if (disconnectedMessageBuffer != null
                    && disconnectedMessageBuffer.getMessageCount() != 0) {
                ...
                if (message instanceof MqttPublish
                        && message.getProperties().getTopicAlias() != null) {
                ...
                if (message instanceof MqttPublish) {
                    ...
                    if (this.mqttConnection.getMaximumQoS() != null
                            && ((MqttPublish) message).getMessage().getQos()
                            > this.mqttConnection.getMaximumQoS()) {
        ...
        synchronized (conLock) {
            if (isDisconnected() && !closePending) {
                ...
                MqttConnect connect = new MqttConnect(client.getClientId(),
                        ...
                        conOptions.getProtocolName(),
                        conOptions.isCleanStart(), conOptions.getKeepAliveIntervalServer(),
                        conOptions.getConnectionProperties(),
                        conOptions.getWillMessageProperties());
...
// Stop the network module, send and receive now not possible
    ...
    public void disconnect(MqttDisconnect disconnect, long quiesceTimeout, MqttToken token)
            throws MqttException {
        ...
        synchronized (conLock) {
            if (isClosed()) {
                ...
                throw ExceptionHelper.createMqttException(
                        MqttClientException.REASON_CODE_CLIENT_DISCONNECTING);
    ...
    public void disconnectForcibly(long quiesceTimeout, long disconnectTimeout, int reasonCode,
        ...
        disconnectForcibly(quiesceTimeout, disconnectTimeout, true, reasonCode,
    ...
    public void setMessageListener(Integer subscriptionId, String topicFilter,
    ...
    private class ConnectBG implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                receiver = new CommsReceiver(clientComms, clientState, tokenStore,
                        networkModule.getInputStream(),
                        networkModule.getSocket(), logger, mqttInterceptorCallback);
                receiver.start("MQTT Rec: " + getClient().getClientId());
                sender = new CommsSender(clientComms, clientState, tokenStore,
                        networkModule.getOutputStream(),
                        networkModule.getSocket(), logger, mqttInterceptorCallback);
    ...
    public MqttMessage getBufferedMessage(int bufferIndex) {
        MqttPublish send = (MqttPublish) this.disconnectedMessageBuffer.getMessage(bufferIndex)
                .getMessage();
