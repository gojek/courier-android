...
import org.eclipse.paho.mqttv5.client.internal.LocalNetworkModule;
...
import org.eclipse.paho.mqttv5.client.internal.SSLNetworkModuleV2;
...
import org.eclipse.paho.mqttv5.client.websocket.WebSocketSecureNetworkModuleV2;
...
//	private Logger log = LoggerFactory.getLogger(LoggerFactory.MQTT_CLIENT_MSG_CAT, CLASS_NAME);
	...
	// Courier customizations: logging, telemetry, experiments and packet interception.
    ...
    {
        this(serverURI, clientId, mqttVersion, persistence, 100, mqttPingSender, logger, new NoOpsPahoEvents(), null, null);
    }
	 ...
	 * @param mqttVersion ignored for MQTT v5 (always 5), retained for API compatibility
	 * @param persistence the persistence class to use to store in-flight messages
	 * @param maxInflightMsgs the maximum number of inflight messages
	 * @param pingSender the {@link MqttPingSender} implementation
	 * @param logger the {@link ILogger} for logging and telemetry
	 * @param pahoEvents the {@link IPahoEvents} for connection lifecycle events
	 * @param experimentsConfig the {@link IExperimentsConfig} for experiment toggles
	 * @param mqttInterceptorList the list of {@link MqttInterceptor}s for wire packet interception
	 * @throws MqttException if any problem was encountered
	 */
	public MqttAsyncClient(
		...
		if (this.pingSender == null) {
			this.pingSender = new TimerPingSender();
		}

		this.persistence.open(clientId);
		this.comms = new ClientComms(this, this.persistence, this.pingSender, this.mqttSession,
				this.mqttConnection, maxInflightMsgs, this.logger, this.experimentsConfig, mqttInterceptorList,
				this.pahoEvents);
	...
	/**
	 * Triggers a Courier fast-reconnect inactivity check on the underlying
	 * client state. If no inbound activity has been seen within the configured
	 * inactivity window the connection will be torn down.
	 *
	...
	private NetworkModule createNetworkModule(String address, MqttConnectionOptions options)
		...
		int serverURIType = MqttConnectionOptions.validateURI(address);

		switch (serverURIType) {
			case MqttConnectionOptions.URI_TYPE_TCP:
				...
				break;
			case MqttConnectionOptions.URI_TYPE_SSL:
				if (experimentsConfig != null && Boolean.TRUE.equals(experimentsConfig.useNewSSLFlow())) {
				...
				break;
			case MqttConnectionOptions.URI_TYPE_WS:
				...
				break;
			case MqttConnectionOptions.URI_TYPE_WSS:
				if (experimentsConfig != null && Boolean.TRUE.equals(experimentsConfig.useNewSSLFlow())) {
				...
				String[] wssEnabledCiphers = wSSFactoryFactory.getEnabledCipherSuites(null);
				if (wssEnabledCiphers != null) {
					((WebSocketSecureNetworkModule) netModule).setEnabledCiphers(wssEnabledCiphers);
				}
				break;
			case MqttConnectionOptions.URI_TYPE_LOCAL:
	...
	private NetworkModule getSSLNetworkModuleV2(String address, MqttConnectionOptions options) throws MqttException {
	...
	private NetworkModule getWSSNetworkModuleV2(String address, MqttConnectionOptions options) throws MqttException {
		String shortAddress = address.substring(6);
		String host = getHostName(shortAddress);
		int port = getPort(shortAddress, 443);
